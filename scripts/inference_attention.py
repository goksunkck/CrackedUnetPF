...
def load_model(model_path, device='cuda'):
    ...
    model = AttentionUNet(n_channels=2, n_classes=3)
...
def visualize_prediction(model, data_dir, idx=150, device='cuda'):
    ...
    plt.title(f"Input Strain Y (Idx {idx})")
    plt.axis('off')
    ...
    output_filename = os.path.join(PROJECT_ROOT, 'outputs', "attention_inference_result.png")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(model_path):
        ...
        visualize_prediction(model, data_dir, idx=400, device=device)
