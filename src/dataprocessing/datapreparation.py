...
class InputData:
    ...
    def read_data_file(self):
        ...
        # Use pandas for faster reading
        # encoding="windows-1252" matches the original genfromtxt
        # delimiter=";" matches original
        # header=None because original seemingly didn't treat first row as special or skipped it manually/implicitly?
        # Actually original used genfromtxt. Let's look at the file format or original code.
        # Original: np.genfromtxt(self.data_file, delimiter=";", encoding="windows-1252")
        # genfromtxt by default treats all lines as data unless skip_header is set.
        # It handles missing values by filling with nan if filling_values is not set? No, genfromtxt default behavior depends.
        # But see _cut_nans: it removes rows with nans.
        
        try:
            df = pd.read_csv(self.data_file, delimiter=";", encoding="windows-1252", header=None)
            np_df = df.to_numpy(dtype=np.float64)
        except Exception:
            # Fallback or error handling if needed, but for now we assume it works as genfromtxt did
            # If there are lines with text headers that cause NaN conversion in float64, they will be Nans.
            # But read_csv might infer types. We force float64 conversion which might fail if there are strings.
            # genfromtxt with dtype=float handles this by making them nan usually or erroring depending on settings.
            # Let's trust that the files are numeric CSVs mostly.
             df = pd.read_csv(self.data_file, delimiter=";", encoding="windows-1252", header=None, comment='#')
             np_df = df.apply(pd.to_numeric, errors='coerce').to_numpy(dtype=np.float64)
